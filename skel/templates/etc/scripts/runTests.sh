    ...
    case ${DBMS} in
        mariadb)
            ...
            if [ "${DATABASE_DRIVER}" != "mysqli" ] && [ "${DATABASE_DRIVER}" != "pdo_mysql" ]; then
                ...
                echo >&2
                echo "Use \"./etc/scripts/runTests.sh -h\" to display help and valid options" >&2
                exit 1
            fi
...
Examples:
    ...
    ./etc/scripts/runTests.sh -x -p 8.2 -e "-v --filter canRetrieveValueWithGP"
    ...
    ./etc/scripts/runTests.sh -x -p 8.2 -s functional -d postgres typo3/sysext/core/Tests/Functional/Authentication

    # Run functional tests on postgres 11
    ./etc/scripts/runTests.sh -s functional -d postgres -i 11
...
USERSET=""
SUFFIX=$(echo $RANDOM)
NETWORK="toujou-container-${SUFFIX}"
...
case ${TEST_SUITE} in
    ...
    composerValidate)
        ...
        ;;
    functional)
        COMMAND=(.Build/bin/phpunit -c etc/FunctionalTests.xml --exclude-group not-${DBMS} "$@")
        case ${DBMS} in
            mariadb)
    ...
    phpstanGenerateBaseline)
        ...
        ;;
    unit)
        ${CONTAINER_BIN} run ${CONTAINER_COMMON_PARAMS} --name unit-${SUFFIX} ${XDEBUG_MODE} -e XDEBUG_CONFIG="${XDEBUG_CONFIG}" ${IMAGE_PHP} .Build/bin/phpunit -c etc/UnitTests.xml "$@"
        SUITE_EXIT_CODE=$?
        ;;
    update)
